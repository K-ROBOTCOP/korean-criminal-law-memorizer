...
import { Book, Layers, GraduationCap, ChevronRight, Search, Shuffle, Activity, Target, Zap, Clock, Trophy } from 'lucide-react';
import criminalActData from './criminal_act.json';

// --- Helper: Flatten Articles & LocalStorage ---
const allArticles = [];
const flatten = (nodes) => {
...
function Dashboard() {
  ...
  return (
    <div className="container">
      <h1 style={{ textAlign: 'center', marginBottom: '16px', background: 'linear-gradient(to right, #60a5fa, #c084fc)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
        형법 암기 마스터
      </h1>
      <p style={{ textAlign: 'center', marginBottom: '48px', fontSize: '1.2rem', color: 'var(--text-secondary)' }}>
        뇌과학과 게임화를 결합한 6단계 입체적 암기 시스템
      </p>
      ...
      <div className="grid-2" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))' }}>
        ...
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <h3>1단계: 구조 및 개요</h3>
          <p style={{ fontSize: '0.9rem', marginBottom: '24px' }}>편, 장, 절의 위치를 파악하고 각 조문의 제목을 플래시카드로 암기합니다.</p>
          <button className="btn btn-primary" style={{ marginTop: 'auto', width: '100%' }} onClick={() => navigate('/phase1')}>
            구조 학습 <ChevronRight size={16} />
          </button>
        </div>
        ...
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '0.9rem', marginBottom: '24px' }}>각 조문의 원문을 읽고, 형량과 핵심 단어를 빈칸 채우기로 완벽하게 암기합니다.</p>
        ...
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
          ...
          <h3>3단계: 랜덤 퀴즈</h3>
          <p style={{ fontSize: '0.9rem', marginBottom: '24px' }}>전체 조문 중 무작위 플래시카드 퀴즈. 틀린 조문은 오답 노트에 저장됩니다.</p>
        ...
        {/* Phase 4 */}
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', border: '1px solid rgba(245, 158, 11, 0.3)' }}>
          <Activity size={40} color="#f59e0b" style={{ marginBottom: '16px' }} />
          <h3>4단계: 핵심 구성요건</h3>
          <p style={{ fontSize: '0.9rem', marginBottom: '24px' }}>문장을 주체, 행위, 형벌 등 블록 단위로 쪼개어 시각적으로 구조를 각인합니다.</p>
          <button className="btn btn-primary" style={{ marginTop: 'auto', width: '100%', backgroundColor: '#f59e0b' }} onClick={() => navigate('/phase4')}>
            구조 분석 <ChevronRight size={16} />
          </button>
        </div>

        {/* Phase 5 */}
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
          <Target size={40} color="#ef4444" style={{ marginBottom: '16px' }} />
          <h3>5단계: 스마트 오답 노트</h3>
          <p style={{ fontSize: '0.9rem', marginBottom: '24px' }}>퀴즈에서 틀렸던 조문만 모아서 반복 훈련합니다. 완벽히 외울 때까지!</p>
          <button className="btn btn-primary" style={{ marginTop: 'auto', width: '100%', backgroundColor: '#ef4444' }} onClick={() => navigate('/phase5')}>
            취약점 극복 <ChevronRight size={16} />
          </button>
        </div>

        {/* Phase 6 */}
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', border: '1px solid rgba(236, 72, 153, 0.3)' }}>
          <Zap size={40} color="#ec4899" style={{ marginBottom: '16px' }} />
          <h3>6단계: 스피드 서바이벌</h3>
          <p style={{ fontSize: '0.9rem', marginBottom: '24px' }}>제한 시간 내에 쏟아지는 조문을 반사적으로 맞히는 타임어택 게임입니다.</p>
          <button className="btn btn-primary" style={{ marginTop: 'auto', width: '100%', backgroundColor: '#ec4899' }} onClick={() => navigate('/phase6')}>
            게임 시작 <ChevronRight size={16} />
          </button>
        </div>
      </div>
    </div>
...
// ... HierarchyNode and Phase1, Phase2 components from before (simplified here to save space, but keeping them intact)
        ...
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
          {selectedArticle ? (
            ...
            <div style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>
              ...
              <p>왼쪽 목차에서 조문을 선택하여 플래시카드를 시작하세요.</p>
            </div>
          )}
        </div>
      </div>
...
function Phase2() {
  ...
  const correctAnswersList = selectedArticle ? (selectedArticle.paragraphs?.length > 0 ? selectedArticle.paragraphs.join(' ') : selectedArticle.content).split(/(\d+년|\d+만원|\d+천만원|사형|무기)/g).filter(p => /(년|만원|천만원|사형|무기)/.test(p)) : [];
...
// --- Phase 3: Random Quiz (Updated with Tracking) ---
function Phase3() {
  ...
  const handleResult = (isCorrect) => {
    if (!isCorrect) addWrong(currentArticle.article_num);
    nextQuiz();
  };
          ...
          <div className="flashcard-container" style={{ maxWidth: '700px' }}>
            <div className={`flashcard ${isFlipped ? 'flipped' : ''}`} style={{ minHeight: '400px' }} onClick={() => !isFlipped && setIsFlipped(true)}>
              ...
              <div className="flashcard-face flashcard-back glass" style={{ border: '2px solid #22c55e', display: 'flex', flexDirection: 'column' }}>
                ...
                <p style={{ marginBottom: '16px' }}>맞히셨나요?</p>
                <div style={{ display: 'flex', gap: '16px' }}>
                  <button className="btn" style={{ background: '#ef4444', color: 'white', padding: '12px 24px' }} onClick={(e) => { e.stopPropagation(); handleResult(false); }}>❌ 틀림 (오답노트 추가)</button>
                  <button className="btn" style={{ background: '#10b981', color: 'white', padding: '12px 24px' }} onClick={(e) => { e.stopPropagation(); handleResult(true); }}>✅ 맞음 (다음)</button>
                </div>
              </div>
            </div>
          </div>
...
// --- Phase 4: Core Elements Analysis ---
function Phase4() {
  const [selectedArticle, setSelectedArticle] = useState(null);

  const analyzeText = (text) => {
    // Simple heuristic parser for legal text
    const words = text.split(' ');
    return words.map((word, i) => {
      if (word.match(/(한\s*자는|한\s*자|경우에는|때에는|인|자가|자에게는)/)) {
        return <span key={i} style={{ background: 'rgba(59, 130, 246, 0.2)', padding: '2px 4px', borderRadius: '4px', borderBottom: '2px solid #3b82f6', marginRight: '4px' }}>{word}</span>;
      }
      if (word.match(/(사형|무기|징역|벌금|과료|구류|몰수|처한다|벌한다|과한다)/)) {
        return <span key={i} style={{ background: 'rgba(239, 68, 68, 0.2)', padding: '2px 4px', borderRadius: '4px', borderBottom: '2px solid #ef4444', marginRight: '4px' }}>{word}</span>;
      }
      if (word.match(/(아니한다|없다|한다|할\s*수\s*있다)/)) {
        return <span key={i} style={{ background: 'rgba(16, 185, 129, 0.2)', padding: '2px 4px', borderRadius: '4px', borderBottom: '2px solid #10b981', marginRight: '4px' }}>{word}</span>;
      }
  ...
  return (
    <div className="container">
      ...
      <div style={{ marginBottom: '16px', display: 'flex', gap: '16px', padding: '16px', background: 'var(--surface-color)', borderRadius: '8px' }}>
        <span style={{ borderBottom: '2px solid #3b82f6' }}>📘 조건/주체 (파란색)</span>
        <span style={{ borderBottom: '2px solid #ef4444' }}>📕 형벌/결과 (빨간색)</span>
        <span style={{ borderBottom: '2px solid #10b981' }}>📗 서술어 (초록색)</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '24px' }}>
        <div className="glass-card" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
          {criminalActData.map((node, idx) => <HierarchyNode key={idx} node={node} onArticleClick={(a) => setSelectedArticle(a)} />)}
        </div>
          ...
          {selectedArticle ? (
            <div className="article-detail">
              ...
              <div style={{ fontSize: '1.2rem', lineHeight: '2' }}>
                {selectedArticle.paragraphs?.length > 0 ? (
                  selectedArticle.paragraphs.map((p, i) => <div key={i} style={{ marginBottom: '16px' }}>{analyzeText(p)}</div>)
                ) : (
                  <div>{analyzeText(selectedArticle.content)}</div>
                )}
              </div>
            </div>
          ) : <div style={{ textAlign: 'center', color: 'var(--text-secondary)' }}><Activity size={64} style={{ margin: '0 auto 16px', opacity: 0.5 }} /><p>조문을 선택하여 구조를 분석하세요.</p></div>}
        </div>
      </div>
    </div>
  );
...
// --- Phase 5: Spaced Repetition (Wrong List) ---
function Phase5() {
  ...
  if (wrongArticles.length === 0) {
    return (
      <div className="container" style={{ textAlign: 'center', padding: '100px 0' }}>
        ...
        <h2>오답 노트가 비어있습니다!</h2>
        <p style={{ color: 'var(--text-secondary)', marginTop: '16px' }}>3단계 랜덤 퀴즈에서 틀린 조문이 이곳에 모입니다.</p>
        <Link to="/phase3" className="btn btn-primary" style={{ marginTop: '24px' }}>3단계로 이동</Link>
      </div>
    );
  }
  ...
  const handleCorrect = () => {
    removeWrong(current.article_num);
    setIsFlipped(false);
  };
        ...
        <div className="flashcard-container" style={{ maxWidth: '700px' }}>
          <div className={`flashcard ${isFlipped ? 'flipped' : ''}`} style={{ minHeight: '400px' }} onClick={() => !isFlipped && setIsFlipped(true)}>
            <div className="flashcard-face flashcard-front glass" style={{ border: '2px solid #ef4444', cursor: 'pointer' }}>
              <Target size={24} color="#ef4444" style={{ position: 'absolute', top: '24px', left: '24px' }} />
              <h2 style={{ fontSize: '3.5rem', margin: '24px 0', color: 'var(--text-primary)' }}>제{current.article_num}조</h2>
              <p style={{ marginTop: 'auto', opacity: 0.7 }}>(클릭하여 정답 확인)</p>
            </div>
            <div className="flashcard-face flashcard-back glass" style={{ border: '2px solid #f59e0b', display: 'flex', flexDirection: 'column' }}>
              ...
              <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '24px', maxHeight: '100px', overflowY: 'auto' }}>
                {current.content}
              </p>
              <div style={{ display: 'flex', gap: '16px', marginTop: 'auto' }}>
                <button className="btn" style={{ background: '#374151', color: 'white', flex: 1 }} onClick={(e) => { e.stopPropagation(); handleWrong(); }}>아직 모르겠음 (다시 복습)</button>
                <button className="btn" style={{ background: '#10b981', color: 'white', flex: 1 }} onClick={(e) => { e.stopPropagation(); handleCorrect(); }}>완벽히 외움 (목록 제거)</button>
              </div>
            </div>
          </div>
        </div>
...
function Phase6() {
  ...
  const handleAnswer = (opt) => {
    if (opt.article_num === currentQuestion.article_num) {
      setScore(s => s + 100);
      setTimeLeft(t => t + 2); // Bonus time
      generateQuestion();
    } else {
      setTimeLeft(t => Math.max(0, t - 5)); // Penalty time
      // Button shake effect could go here
    }
  };
  ...
  if (!isPlaying && !gameOver) {
    return (
      <div className="container" style={{ textAlign: 'center', padding: '100px 0' }}>
        ...
        <h2 style={{ fontSize: '2.5rem', marginBottom: '16px' }}>스피드 서바이벌</h2>
        <p style={{ fontSize: '1.2rem', color: 'var(--text-secondary)', marginBottom: '32px' }}>
          60초 안에 최대한 많은 조문의 제목을 맞히세요!<br/>
          정답 시 +2초, 오답 시 -5초 페널티가 있습니다.
        </p>
  ...
  if (gameOver) {
    return (
      <div className="container" style={{ textAlign: 'center', padding: '100px 0' }}>
        ...
        <h2 style={{ fontSize: '2.5rem', marginBottom: '16px' }}>게임 종료!</h2>
        <h3 style={{ fontSize: '3rem', color: '#ec4899', marginBottom: '32px' }}>최종 점수: {score}점</h3>
        <div style={{ display: 'flex', justifyContent: 'center', gap: '16px' }}>
          <button className="btn btn-primary" onClick={startGame}>다시 하기</button>
          <Link to="/" className="btn btn-secondary">메인으로</Link>
        </div>
      </div>
    );
  }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
          {options.map((opt, i) => (
            <button 
              ...
              style={{ padding: '24px', fontSize: '1.2rem', height: 'auto', background: 'rgba(255,255,255,0.1)' }}
...
// ... SearchBar and ArticleDetail components remain unchanged ...
function ArticleDetail({ articleNum }) {
  ...
  return (
    <div className="container">
      ...
      <div className="glass-card">
        <div className="article-detail">
          <h3 style={{ fontSize: '1.8rem', marginBottom: '24px', color: 'var(--accent-color)', borderBottom: '1px solid var(--surface-border)', paddingBottom: '12px' }}>제{article.article_num}조({article.title})</h3>
          {article.paragraphs?.length > 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>{article.paragraphs.map((p, i) => <div key={i} className="article-content">{p}</div>)}</div>
...
function SearchBar() {
  ...
  useEffect(() => {
    ...
    const filtered = allArticles.filter(a => a.article_num.includes(lowerQuery) || (a.title && a.title.toLowerCase().includes(lowerQuery)) || (a.content && a.content.toLowerCase().includes(lowerQuery)) || (a.paragraphs && a.paragraphs.some(p => p.toLowerCase().includes(lowerQuery)))).slice(0, 10);
    setResults(filtered); setIsOpen(true);
  }, [query]);

  return (
    <div ref={searchRef} style={{ position: 'relative', width: '300px' }}>
      <div style={{ position: 'relative' }}>
        <input type="text" placeholder="조문 검색 (예: 250, 살인...)" value={query} onChange={(e) => setQuery(e.target.value)} onFocus={() => { if(query) setIsOpen(true) }} style={{ width: '100%', padding: '10px 16px 10px 40px', borderRadius: '20px', border: '1px solid var(--surface-border)', background: 'rgba(255, 255, 255, 0.05)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit' }} />
      ...
      {isOpen && results.length > 0 && (
        <div className="glass" style={{ position: 'absolute', top: '100%', left: 0, right: 0, marginTop: '8px', maxHeight: '400px', overflowY: 'auto', zIndex: 100, padding: '8px' }}>
          {results.map(a => (
            <div key={a.article_num} onClick={() => { setIsOpen(false); setQuery(''); navigate(`/article/${a.article_num}`); }} style={{ padding: '12px', cursor: 'pointer', borderRadius: '8px', transition: 'background 0.2s', borderBottom: '1px solid rgba(255,255,255,0.05)' }} onMouseOver={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.1)'} onMouseOut={(e) => e.currentTarget.style.background = 'transparent'}>
              <div style={{ color: 'var(--accent-color)', fontWeight: 'bold', marginBottom: '4px' }}>제{a.article_num}조</div>
              <div style={{ color: 'var(--text-primary)', fontSize: '0.95rem' }}>{a.title}</div>
            </div>
          ))}
        </div>
      )}
        ...
        <nav className="navbar">
          <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <Link to="/" style={{ textDecoration: 'none', color: 'white', display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <span style={{ fontSize: '1.2rem', fontWeight: 'bold', letterSpacing: '-0.5px' }}>형법마스터</span>
            </Link>
            <SearchBar />
            <div className="nav-links">
              {/* Using dropdown or simple links depending on space, simplified here */}
